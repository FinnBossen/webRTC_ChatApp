            ...
            <IonPage className='loginWindow page'>
                <IonContent color='primary'>
                    <IonPage className='loginForm'>
                        ...
                        <IonRow>
                        <TextareaAutosize className='myNameChanger'  onChange={(input) => this.setState({   myNameInput: input.target.value})} value={this.state.textInput}     rows={1} maxRows={1} />
                        </IonRow>
                                <IonRow>
                            <IonButton className='loginButton' size="large" expands="block" fill="clear" color="secondary" onClick ={() => this.props.diLogin(this.state.myNameInput)} disabled={this.state.myNameInput === ''}>
