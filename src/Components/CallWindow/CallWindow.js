...
import { volumeMuteSharp, desktopSharp, personAdd, sadSharp, trashBinSharp, callSharp, closeCircleSharp} from "ionicons/icons";
...
class CallWindow extends Component  {
    ...
    render() {
        return (
                <Resizable size={{ width: this.state.initialSize.x, height: this.state.initialSize.y}}  onResizeStop={(e, direction, ref, d) => {
                    this.setState({
                        initialSize: {x:this.state.initialSize.x + d.width, y:this.state.initialSize.y + d.height}
                    });
                }} className='CallWindow' id='CallWindowID' >
                <IonPage>
                    <IonContent className='call-content' id='call-content-ID' onClick={this.toggleMove}  onMouseMove={this.showCoordinates} onMouseLeave={() => {this.toggleMove(true)}}>
                        {this.state.isOpener?<div className='otherStreamsContainer'>
                            {!this.state.isAnswered&&<div className='loadingSpinnerContainer'><p className='loadingText'>Is waiting for answer</p><IonSpinner name="crescent" className='loadingSpinner'/></div>}
                        </div>:
                            <div className='CallOfferContainer'>
                                <IonButtons className='CallOfferButtons-wrapper'>
                                    <IonButton className='accept-btn' size="large" expands="block" fill="clear" color="primary" onClick ={this.acceptCall} disabled={this.state.screenShare}>
                                        <IonIcon slot="icon-only" icon={callSharp} />
                                    </IonButton>
                                    <IonButton className='decline-btn'  size="large" expands="block" fill="clear" color="primary" onClick ={() => { this.props.hangUpCall(this.state.targetUser, true)}}>
                    ...
                    <IonFooter className="footer-chat">
                        <IonToolbar color="light">
                            <IonButtons className='button-wrapper'>
                                <IonButton className='mute-btn' size="large" expands="block" fill="clear" color="primary" onClick ={this.toggleAudio} disabled={this.state.screenShare}>
                                    <IonIcon slot="icon-only" icon={volumeMuteSharp} />
                                </IonButton>
                                <IonButton className='desktop-btn'  size="large" expands="block" fill="clear" color="primary" onClick ={this.toggleStreamType}>
                                    <IonIcon slot="icon-only" icon={desktopSharp} />
                                </IonButton>
                                <IonButton className='add-btn'  size="large" expands="block" fill="clear" color="primary" onClick ={this.addCallMember}  disabled={true}>
                                    <IonIcon slot="icon-only" icon={personAdd} />
                                </IonButton>
                                <IonButton className='face-btn'  size="large" expands="block" fill="clear" color="primary" onClick ={this.toggleVideo}>
                                    <IonIcon slot="icon-only" icon={sadSharp} />
                                </IonButton>
                                <IonButton className='cancel-btn'  size="large" expands="block" fill="clear" color="primary" onClick ={() => this.removeTracks(this.state.targetUser, true)}>
